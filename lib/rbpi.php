...
class Rbpi {
    ...
    public static function hostname($full = false) {
        global $ssh;
        return $full ? $ssh->shell_exec_noauth("hostname -f") : $ssh->shell_exec_noauth("hostname");
    }

    public static function internalIp() {
        global $ssh;
        $ip = $ssh->shell_exec_noauth("ip addr | grep \"inet \" | grep brd | perl -p -e 's/^.*inet (.*)\/.*$/$1/g' | tail -n-1");
        return $ip != "" ? $ip : $_SERVER['SERVER_ADDR'];
